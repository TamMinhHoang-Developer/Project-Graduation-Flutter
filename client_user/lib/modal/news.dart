...
class NewsSnapshot {
  ...
  static Stream<List<List<NewsSnapshot>>> getListNewsByUserGroupByDate(
    ...
    return listDocSnap.map((listDocSnap) {
      ...
      final sortedGroupedNews = groupedNews.values.toList()
        ..sort((a, b) {
          final aMaxDate = a
              .map((newsSnapshot) => newsSnapshot.orderDetail!.CreateAt!)
              .reduce((value, element) => value > element ? value : element);
          final bMaxDate = b
              .map((newsSnapshot) => newsSnapshot.orderDetail!.CreateAt!)
              .reduce((value, element) => value > element ? value : element);
          return bMaxDate
              .compareTo(aMaxDate); // Sắp xếp theo thứ tự giảm dần của ngày
        });

      sortedGroupedNews.forEach((group) {
        group.sort((a, b) =>
            b.orderDetail!.CreateAt!.compareTo(a.orderDetail!.CreateAt!));
      });

      return sortedGroupedNews;
    });
  }
